...
export default function PulseXSeries() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            const data = await fetchProducts({ category: 'Pulse X Series' });
            if (mounted) {
                setProducts(data);
                setLoading(false);
            }
    ...
    return (
        <main className="container mx-auto py-12 px-4">
            <h1 className="text-3xl font-bold mb-2">Pulse X Series</h1>
            <p className="text-sm text-slate-500 mb-8">Live inventory from Supabase category: Pulse X Series</p>
            ...
            {loading ? (
                ...
                <div className="bg-slate-100 rounded-xl p-8 text-slate-700">
                    <p>No Pulse X Series products found.</p>
                </div>
            ) : (
